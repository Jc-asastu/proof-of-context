...
{\small\noindent\textbf{Version 0.8} --- \textbf{Published: 20 June 2026} (revision from v0.7: \S10 \emph{Reference Implementation} updated to the v0.3 crate --- the gate now enforces $\mathrm{consistent} + \mathrm{fresh}_m + \mathrm{fresh}_i + \mathrm{fresh}_s$ (integrity plus three of the four freshness types) against the canonical-state oracle of Proposition~2, realized in two real forms: a witness-presented BaseOracle attestation verifier for $\mathrm{fresh}_i$, and an M-of-N committee/quorum model-lineage registry for $\mathrm{fresh}_m$. $\mathrm{consistent}$ is refined throughout \S9 (including Theorem~1 and the FGSS game) to internal triple-anchor agreement --- a property of $A$ alone, with the staleness guarantee resting on $\mathrm{fresh}_s$ and $\mathrm{consistent}$ foreclosing a fabricated-fresh anchor --- and the $\mathrm{fresh}_c$ measurability overclaim is corrected: $\mathrm{now}.h - A.h$ is $\mathrm{fresh}_s$, not $\mathrm{fresh}_c$, handled structurally via commit-at-completion and omitted from the gate. Adds \S10.5 on a multi-party on-chain instantiation in an agent-to-agent dark pool, and cross-language test vectors for the $f_i$/$f_m$ roots. The proved soundness of the enforced $\{\mathrm{consistent}, \mathrm{fresh}_s\}$ subset is unchanged in substance. No v0.7 theorem is retracted. --- Prior revision from v0.6 added \S9 \emph{Formal Model and Soundness} and \S10; former \S9--\S11 were renumbered to \S11--\S13.)\par}
...
\begin{quote}
Maisu, J. C. (2026). \emph{Proof-of-Context: The Missing Verification Layer in Decentralized ML Protocols} (Version 0.8) [Position paper]. github.com/asastuai/proof-of-context, 20 June 2026.
\end{quote}
