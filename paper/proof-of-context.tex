...
\href{mailto:juancmaisu@outlook.com}{juancmaisu@outlook.com} \quad\textperiodcentered\quad \href{https://github.com/asastuai}{github.com/asastuai}}
\date{}
...
{\small\noindent\textbf{Version 0.7} --- \textbf{Published: 11 June 2026} (revision from v0.6: adds \S9 \emph{Formal Model and Soundness} --- precise definitions, the freshness-gated settlement soundness game, Theorem~1 for the enforced subset under collision-resistance + EUF-CMA + attestation soundness + an explicit honest-clock assumption, and Proposition~2 for the oracle-dependent extension to model- and input-freshness --- and \S10 \emph{Reference Implementation} --- the v0.2 crate with real SHA-256/Ed25519, 33 passing tests, and measured primitive costs. Former \S9--\S11 renumbered to \S11--\S13; section references in revision notes for v0.6 and earlier use the numbering in effect at that revision. No v0.6 claim is retracted.)\par}
...
\]
where $\mathrm{enc}(\cdot)$ is the fixed-width encoding of the structured fields and the kv-cache component is present only when the inference uses a persistent cache. The canonical preimage is 251 bytes without the kv-cache and 283 with it.
...
\begin{itemize}
\item $\mathrm{consistent}(C, \mathrm{now}, \theta)$: the commit anchor agrees with \texttt{now} across the three clocks within skew, $\neg\mathrm{diverge}_\theta(A, \mathrm{now})$. This is an \emph{integrity} check on the anchor --- do the three clocks corroborate each other --- and is distinct from the staleness horizons below.
\item $\mathrm{fresh}_c(C, \mathrm{now}, \theta)$: the result was not held back, $\mathrm{now}.h - A.h \le \theta.\mathrm{max\_fc}$ (the computational-recency horizon).
...
A settlement gate is the predicate $G(C, \mathrm{now}, \theta) = \mathrm{Verify}(C) \wedge \mathrm{consistent} \wedge \mathrm{fresh}_c \wedge \mathrm{fresh}_m \wedge \mathrm{fresh}_i \wedge \mathrm{fresh}_s$, where $\mathrm{Verify}(C)$ checks the Ed25519 signature over $d$ and validates the attestation chain $\chi$. When $G$ holds the gate returns \texttt{Clear}; otherwise it returns the set of violated predicates, so the protocol can emit the corresponding refund or slash event. Among the freshness predicates only $\mathrm{fresh}_m$ and $\mathrm{fresh}_i$ require state external to the commitment --- a canonical model root and a canonical input-world state at settlement time; the integrity check $\mathrm{consistent}$, together with $\mathrm{fresh}_c$ and $\mathrm{fresh}_s$, is decidable from $C$ and \texttt{now} alone. We isolate the external dependency deliberately: it is the formal residue of the DeFi oracle-freshness discipline imported in \S5.
...
\emph{$\mathrm{fresh}_m$ and $\mathrm{fresh}_i$ are only as sound as their oracle.} The reference gate does not enforce them (\S10); a gate that did would need a canonical model root and input-world state at settlement time. $\mathrm{fresh}_c$, by contrast, needs no external state --- it is a block-height delta like $\mathrm{fresh}_s$ --- and is simply not yet wired into the reference gate (see \S10.2).
...
The model of \S9 is not aspirational: its objects, predicates, and the enforced subset of the gate are implemented in a reference crate, \texttt{proof-of-context} (v0.2, MIT OR Apache-2.0). The crate is pure Rust with \texttt{\#![forbid(unsafe\_code)]} and depends only on \texttt{sha2} and \texttt{ed25519-dalek} for cryptography. This section reports what the implementation establishes and, as importantly, what it does not yet enforce.
...
\paragraph{10.2 What it enforces, and what it does not.} A strict reading of the gate is warranted, because the gap between ``the four types are defined'' and ``the four types gate payment'' is precisely the overstatement this program avoids. The reference gate computes $G_{\mathrm{impl}} = \mathrm{Verify} \wedge \mathrm{consistent} \wedge \mathrm{fresh}_s$: signature-and-attestation validity; three-clock consistency $\neg\mathrm{diverge}_\theta$ (a violation is reported, coarsely, under the computational-freshness type --- \texttt{mock.rs} flags that a finer per-axis attribution is future work); and the settlement window $\mathrm{fresh}_s$. It does \emph{not} enforce the other three freshness predicates, and the two reasons differ. $\mathrm{fresh}_c$ (the \texttt{max\_fc} recency horizon) needs \textbf{no external state} --- it is a block-height delta exactly like $\mathrm{fresh}_s$, simply not yet wired into the gate (\texttt{max\_fc\_blocks} is defined in the threshold struct but unread); adding it is a few lines. $\mathrm{fresh}_m$ and $\mathrm{fresh}_i$, by contrast, require the \textbf{canonical-state oracle} of Proposition~2 (H4) and are deferred to Phase 3. The implementation therefore realizes the enforced subset of Theorem~1 --- $\mathrm{consistent}$ and $\mathrm{fresh}_s$ --- and \emph{commits to, but does not gate on,} the recency, model, and input channels. The root binds the model-weights hash and the input-manifest root, so the data needed to \emph{query} $\mathrm{fresh}_m$/$\mathrm{fresh}_i$ against canonical state is committed; the epoch-distance mapping itself --- which model version a weights hash denotes --- is part of the deferred oracle, not of the commitment. Two honest caveats on the reference gate. First, binding assumes an injective field encoding: the \texttt{AttentionImpl::Other(tag)} discriminant currently folds an 8-bit tag into seven bits (\texttt{0x80 | (tag \& 0x7F)}), so two \texttt{Other} tags differing only in the high bit produce the same root --- a known non-injectivity (the enumerated \texttt{FlashAttention2}/\texttt{Sdpa}/\texttt{FlexAttention} variants are unaffected), and exactly the trivial-evasion vector \S8 warns of; fixing it changes canonical hashes and is therefore coordinated with the wire-format specification. Second, on a backwards-going clock (\texttt{now.h < A.h}) the gate short-circuits to a settlement-freshness rejection rather than accumulating every violated predicate.

\paragraph{10.3 What the tests establish.} The suite (33 tests, all passing on the default feature set; live Drand and Base-RPC anchor tests are network-gated and excluded from that count) exercises three claims relevant to the model:
...
We publish this paper to (a) establish the term \emph{proof-of-context}, the attestation-as-settlement distinguishing axis, the four-freshness-type decomposition, the execution-context-root scope, and the cross-domain DeFi analogy on the academic record (priority date 22 April 2026, the original position-paper publication; this is revision v0.7 of 11 June 2026); (b) situate the contribution honestly against the substantial existing work in staleness-aware federated learning [5--9], proof-of-learning robustness [1--3, 10], centralized-provider property attestation [17], inference-activation LSH [16], and cryptoeconomic-offensive staleness [19]; and (c) invite protocol teams, independent researchers, and cryptographic-primitive designers to begin treating proof-of-context as a first-class composable settlement primitive rather than a collection of phase-specific or differently-motivated concerns.
...
\begin{quote}
Maisu, J. C. (2026). \emph{Proof-of-Context: The Missing Verification Layer in Decentralized ML Protocols} (Version 0.7) [Position paper]. github.com/asastuai/proof-of-context, 11 June 2026.
\end{quote}
