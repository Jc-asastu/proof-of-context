...
**Version 0.7** — **Published: 11 June 2026** (revision from v0.6: adds §9 *Formal Model and Soundness* — precise definitions, the freshness-gated settlement soundness game, Theorem 1 for the enforced subset under collision-resistance + EUF-CMA + attestation soundness + an explicit honest-clock assumption, and Proposition 2 for the oracle-dependent extension to model- and input-freshness — and §10 *Reference Implementation* — the v0.2 crate with real SHA-256/Ed25519, 33 passing tests, and measured primitive costs. Former §9–§11 renumbered to §11–§13; section references in revision notes for v0.6 and earlier use the numbering in effect at that revision. No v0.6 claim is retracted.)
...
The model of §9 is not aspirational: its objects, predicates, and the enforced subset of the gate are implemented in a reference crate, `proof-of-context` (v0.2, MIT OR Apache-2.0). The crate is pure Rust with `#![forbid(unsafe_code)]` and depends only on `sha2` and `ed25519-dalek` for cryptography. This section reports what the implementation establishes and, as importantly, what it does not yet enforce.
...
**10.2 What it enforces, and what it does not.** A strict reading of the gate is warranted, because the gap between "the four types are defined" and "the four types gate payment" is precisely the overstatement this program avoids. The reference gate computes `G_impl = Verify ∧ consistent ∧ fresh_s`: signature-and-attestation validity; three-clock consistency `¬diverge_θ` (a violation is reported, coarsely, under the computational-freshness type — `mock.rs` flags that a finer per-axis attribution is future work); and the settlement window `fresh_s`. It does *not* enforce the other three freshness predicates, and the two reasons differ. `fresh_c` (the `max_fc` recency horizon) needs **no external state** — it is a block-height delta exactly like `fresh_s`, simply not yet wired into the gate (`max_fc_blocks` is defined in the threshold struct but unread); adding it is a few lines. `fresh_m` and `fresh_i`, by contrast, require the **canonical-state oracle** of Proposition 2 (H4) and are deferred to Phase 3. The implementation therefore realizes the enforced subset of Theorem 1 — `consistent` and `fresh_s` — and *commits to, but does not gate on,* the recency, model, and input channels. The root binds the model-weights hash and the input-manifest root, so the data needed to *query* `fresh_m`/`fresh_i` against canonical state is committed; the epoch-distance mapping itself — which model version a weights hash denotes — is part of the deferred oracle, not of the commitment. Two honest caveats on the reference gate. First, binding assumes an injective field encoding: the `AttentionImpl::Other(tag)` discriminant currently folds an 8-bit tag into seven bits (`0x80 | (tag & 0x7F)`), so two `Other` tags differing only in the high bit produce the same root — a known non-injectivity (the enumerated `FlashAttention2`/`Sdpa`/`FlexAttention` variants are unaffected), and exactly the trivial-evasion vector §8 warns of; fixing it changes canonical hashes and is therefore coordinated with the wire-format specification. Second, on a backwards-going clock (`now.h < A.h`) the gate short-circuits to a settlement-freshness rejection rather than accumulating every violated predicate.

**10.3 What the tests establish.** The suite (33 tests, all passing on the default feature set; live Drand and Base-RPC anchor tests are network-gated and excluded from that count) exercises three claims relevant to the model:
...
- *The rejection paths fire.* A commitment whose Drand round has advanced beyond the skew tolerance is `Rejected([Computational])`; one settled beyond `max_fs` is `Rejected([Settlement])` (the result carries the *set* of violated freshness types); a flipped signature byte or a post-signing change to the output hash is `InvalidSignature`.
- *The signed object round-trips.* A commitment serializes and deserializes without loss and re-verifies — the property the wire-format specification depends on for cross-implementation interoperability.
