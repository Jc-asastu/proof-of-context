...
**Version 0.9** — **Published: 21 June 2026** (revision from v0.8: adds **§9.6 + Corollary 1** — *multi-party FGSS soundness*, the dark-pool gate `G^N` proven sound by a per-party union bound over Theorem 1 — and a **§10.6 Evaluation** reporting measured costs: a full multi-party dark-pool settlement consumes 120,799 compute units (≈8.6% of Solana's per-tx budget), the off-chain oracle paths run in 1–53 µs, the suites pass 84 (crate) + 96 (on-chain) tests, and the canonical-JSON roots converge byte-identically across Rust/JS/TS. The §10.4 cost table is extended with the oracle-path figures. No prior theorem is retracted. — *Prior (v0.8, from v0.7):* §10 *Reference Implementation* updated to the v0.3 crate — the gate now enforces `consistent + fresh_m + fresh_i + fresh_s` (integrity plus three of the four freshness types) against the canonical-state oracle of Proposition 2, realized in two real forms: a witness-presented BaseOracle attestation verifier for `fresh_i`, and an M-of-N committee/quorum model-lineage registry for `fresh_m`. `consistent` is refined throughout §9 (including Theorem 1 and the FGSS game) to **internal triple-anchor agreement** — a property of `A` alone, with the staleness guarantee resting on `fresh_s` and `consistent` foreclosing a fabricated-fresh anchor — and the `fresh_c` measurability overclaim is corrected: `now.h − A.h` is `fresh_s`, not `fresh_c`, which is handled structurally via commit-at-completion and omitted from the gate. Adds §10.5 on a multi-party on-chain instantiation in an agent-to-agent dark pool, and cross-language test vectors for the `f_i`/`f_m` roots. The proved soundness of the enforced `{consistent, fresh_s}` subset is unchanged in substance — the restatement localizes the staleness guarantee onto `fresh_s` and recasts `consistent` as clock-tamper detection. No v0.7 theorem is retracted. — Prior revision from v0.6 added §9 *Formal Model and Soundness* (definitions, the freshness-gated settlement soundness game, Theorem 1 for the enforced subset, Proposition 2 for the oracle-dependent extension) and §10; former §9–§11 were renumbered to §11–§13.)
