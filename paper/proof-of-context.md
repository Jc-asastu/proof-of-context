...
v0.9 adds **§9.6 + Corollary 1** — *multi-party FGSS soundness*, the dark-pool gate `G^N` proven sound by a per-party union bound over Theorem 1 — and a **§10.6 Evaluation** with measured costs: a full multi-party dark-pool settlement consumes 120,799 compute units (≈8.6% of Solana's per-tx budget), the off-chain oracle paths run in 1–53 µs, the suites pass 84 (crate) + 96 (on-chain) tests, and the canonical-JSON roots converge byte-identically across Rust/JS/TS. v0.9.1 corrects stale in-text version references and records the resolution of the §10.2 `Other`-tag non-injectivity in crate v0.3.1. No prior theorem is retracted; full history in the Revision note.
...
*Execution-context root.* The root is a hash commitment over the ten components enumerated in §8: `R = H(weights ‖ tokenizer ‖ system_prompt ‖ enc(sampling) ‖ runtime ‖ attn ‖ precision ‖ enc(inference_config) ‖ input_manifest ‖ [kv_cache])`, where `enc(·)` is the fixed-width encoding of the structured fields and the kv-cache component is present only when the inference uses a persistent cache. The canonical preimage is 251 bytes without the kv-cache and 283 with it (one byte more in each case when the attention implementation is an `Other` tag, whose injective encoding is the two-byte sequence `0x80 ‖ tag` — see §10.2).
...
- `consistent(C, θ)`: the commit anchor's three clocks corroborate *each other* within skew — the block-derived, TEE, and Drand-derived wall-times of `A` mutually agree. This is an *integrity* check on the anchor **alone** (a function of `A`, not of `A` versus `now`): it detects a tampered or desynced commit clock and, crucially, forecloses a worker presenting a block height out of line with its TEE and Drand readings. It is distinct from — and does not by itself bound — the staleness horizons below.
- `fresh_c(C, θ)`: the computation was not held back between completion and commitment. This is the one type **not decidable from `(A, now)`**: `now.h − A.h` is the commit→settle gap (that is `fresh_s`), not the compute→commit gap, which would require a computation-completed timestamp the commitment does not carry. We therefore treat `f_c` *structurally* (commit-at-completion; see §10.2) rather than as a gate predicate, and the gate below omits it.
...
A settlement gate is the predicate `G(C, now, θ) = Verify(C) ∧ consistent ∧ fresh_m ∧ fresh_i ∧ fresh_s`, where `Verify(C)` checks the Ed25519 signature over `d` and validates the attestation chain `χ`. When `G` holds the gate returns `Clear`; otherwise it returns the set of violated predicates, so the protocol can emit the corresponding refund or slash event. (`f_c` is absent by design — handled structurally per §10.2, not gated.) Among the freshness predicates only `fresh_m` and `fresh_i` require state external to the commitment — a canonical model root and a canonical input-world state at settlement time; the integrity check `consistent` is decidable from `C` alone and `fresh_s` from `C` and `now`. We isolate the external dependency deliberately: it is the formal residue of the DeFi oracle-freshness discipline imported in §5.
...
*Theorem 1 (soundness of the enforced subset).* Under (H1), (H2), (H3a), and (H3b), the gate of §9.2 restricted to the predicates it enforces — the anchor-integrity check `consistent` and the settlement window `fresh_s` — is FGSS-sound. Concretely, if `G(C, now, θ) = Clear`, then — except with probability negligible in `λ`, the collision/forgery slack of (H1) and (H2) — the commitment's `(R, A, y)` is authentic; under (H3b) the committed anchor `A` is then the genuine reading `A*`; and the gate's explicit checks force that `A`'s three clocks mutually corroborate within skew (`consistent`) and that `now.h − A.h ≤ θ.max_fs` (`fresh_s`). Because `consistent` ties the committed `A.h` to the (honest, by (H3b)) TEE and Drand readings, a within-window `A.h` cannot be a fabricated-fresh height; the staleness guarantee thus rests on `fresh_s`, with `consistent` foreclosing the fabricated-anchor evasion.

*Proof (sketch).* (1) **Binding.** By (H1), and assuming the field encoding is injective (see §10.2 for one implementation caveat), `R` is a binding commitment to the ten context components and `d` to `(R, A, y)`; an adversary clearing payment under a different context, anchor, or output exhibits a collision in `H`. (2) **Authenticity and truthfulness of the anchor.** A `Clear` requires `Verify(C)`, so `σ` is a valid signature over `d` under `pk`. By (H2) the signature cannot be forged without the key, and by (H3a) the key belongs to a known-good enclave, so the binding of `A` into `d` is authentic. The stronger claim `A = A*` — that the committed anchor is the *genuine* clock reading rather than one chosen to look fresh — is supplied by (H3b), not by EUF-CMA or attestation; it is the load-bearing assumption of this step, and the one §9.5 removes to show where the guarantee ends. (3) **Gating.** Given `A = A*` and a `Clear`, the gate's explicit checks force `consistent(A)` — `A`'s block-, TEE-, and Drand-derived times mutually agree within skew, so the committed `A.h` is corroborated by the honest (by (H3b)) TEE and Drand readings and cannot be a fabricated-fresh height — and `fresh_s`: `now.h − A.h ≤ θ.max_fs`. A genuinely stale result (`now.h − A*.h > θ.max_fs`) therefore cannot clear; any `C` violating either check is `Rejected`, contradicting `Clear`. ∎
...
*`fresh_m` and `fresh_i` are only as sound as their oracle.* They rest on the canonical-state oracle (H4); the reference gate now enforces them against that oracle (§10), realized in mock and real forms. `fresh_c`, by contrast, is **not** a settlement-decidable delta: `now.h − A.h` is `fresh_s`, not `f_c` (which names the compute→commit gap and would need a completion timestamp the commitment lacks). It is handled *structurally* — a commit-at-completion committer makes `f_c ≡ 0` by construction, and a late submission of the signed result is caught by `fresh_s` (see §10.2) — rather than enforced as a gate predicate.
...
**9.6 Multi-party gating.** The agent-economy deployment of §10.5 settles a *negotiated* trade between `N` parties, each presenting its own commitment `C_k` against a shared settlement-time state. Define the multi-party gate `G^N({C_k}, now, θ) = ⋀_{k=1}^{N} G(C_k, now, θ)`: the trade clears iff every party's commitment clears the single-party gate. The corresponding win condition generalizes pointwise — 𝒜 wins if `G^N = Clear` while *some* party `k`'s genuine context is stale.

*Corollary 1 (multi-party FGSS).* Under the assumptions of Theorem 1 (resp. Proposition 2), `G^N` is FGSS-sound: `Pr[𝒜 wins the N-party game] ≤ N · negl(λ) = negl(λ)`. *Proof.* If `G^N = Clear` then `G(C_k, now, θ) = Clear` for every `k`; a win requires some party `k*` whose genuine context is stale yet `G(C_{k*}, now, θ) = Clear`, which is exactly a single-party FGSS win for `k*`. By Theorem 1 (resp. Proposition 2) each such event has probability `≤ negl(λ)`; a union bound over the `N` parties preserves negligibility for any `N = poly(λ)`. ∎ The reduction is deliberately trivial: multi-party gating buys no new soundness risk, because the per-party guarantee composes by conjunction. What it *adds* is per-party *attribution* — the gate returns which party (and which freshness type) failed — so the protocol can refund or slash the responsible agent rather than voiding the trade wholesale.

---

## 10. Reference Implementation

The model of §9 is not aspirational: its objects, predicates, and the gate are implemented in a reference crate, `proof-of-context` (v0.3.1, MIT OR Apache-2.0). The crate is pure Rust with `#![forbid(unsafe_code)]`; its default build depends only on `sha2` and `ed25519-dalek`, and the canonical-state oracles of Proposition 2 ship as opt-in feature-gated modules. This section reports what the implementation establishes and, as importantly, what it does not yet enforce.
...
- *`consistent`* — refined to **internal triple-anchor agreement**: the commit anchor's TEE- and Drand-derived wall-times (and, under the live-anchor feature, the block-derived time) corroborate *each other* within skew. This is a property of the commit anchor `A` alone — it detects a tampered or desynced commit clock — and replaces the `A`-vs-`now` form of earlier drafts, which conflated the integrity check with the settlement horizon (that form at a tight skew makes `fresh_s` unreachable). §9.2 and Theorem 1 now carry this refinement: the staleness guarantee rests on `fresh_s`, and `consistent` forecloses a fabricated-fresh anchor.
...
The one disclosed non-injectivity in the reference root is resolved as of crate v0.3.1: the `AttentionImpl::Other(tag)` discriminant previously folded an 8-bit tag into seven bits (`0x80 | (tag & 0x7F)`), so two `Other` tags differing only in the high bit produced the same root — exactly the trivial-evasion vector §8 warns of. `Other` now encodes as the two-byte sequence `0x80 ‖ tag`, restoring injectivity; the enumerated `FlashAttention2`/`Sdpa`/`FlexAttention` variants keep their single-byte encodings, so roots committed with enumerated variants are unchanged, and no published test vector had used `Other`. Regression tests pin the previously-colliding pair. (The v0.2 backwards-clock short-circuit is likewise resolved: the gate accumulates every violated predicate rather than returning early.)
...
The oracle paths are the same order of magnitude, dominated by Ed25519 verification: the canonical-JSON hash (shared by the `f_i` and `f_m` roots) runs in about 1.1 µs, an `f_i` BaseOracle witness verification in about 26 µs (one signature check), an `f_m` 2-of-3 quorum lineage adoption in about 53 µs (two signature checks plus parsing), and the full multi-party dark-pool gate in about 25 µs per party.[^bench] None approaches the cost of the inference being gated.

[^bench]: AMD Ryzen 7 9800X3D, rustc 1.94.0, release profile, ed25519-dalek 2.x, 200,000 iterations per primitive via the `measure_costs` example in the reference crate (oracle-path figures with `--features "oracle-fi oracle-fm darkpool-sol"`). Figures indicate order of magnitude, not a benchmark claim against alternative constructions.

**10.5 A concrete on-chain instantiation: the agent-to-agent dark pool.** The reference crate is venue-neutral; to show the primitive gating real settlement, it is instantiated on-chain in an agent-to-agent (A2A) perp dark pool — a market in which one agent posts an *intent* (a price band) and another posts a *response* (a price), and a negotiated trade clears only when both accept. This is precisely a setting where *proof-of-context* bites: each agent's quoted price is the output of reasoning over a market context (the price/oracle state it observed), and a trade negotiated against stale context is economically toxic. The deployment adds, inside the dark pool's settlement instruction, an **`f_i` gate** read trustlessly from chain state — the canonical market price's last-update timestamp must be within a freshness budget of the settlement clock (`now − last_price_update ≤ budget`), filling a gap where the price was written but never checked at trade time — and binds to each quote a 32-byte **context commitment** (the hash of the off-chain context the agent reasoned over), authenticated by the agent's own transaction signature. `f_s` is supplied natively by the intent/response expiry windows; `f_m`/`f_c` are deferred at this layer. The construction generalizes the single-commitment gate of §9 to a **multi-party** one (formalized as `G^N` in §9.6, with Corollary 1 reducing its soundness to a per-party application of Theorem 1): settlement clears only if *every* negotiating party's quote was made against fresh context — the natural form for the agent economy, where the parties to a trade each carry their own context. The on-chain code is an engineering realization of `G^N`; it is reported here as evidence that the gate composes into a live settlement path rather than only a standalone verifier.

**10.6 Evaluation.** Three measurements ground the claims above.

- *On-chain cost.* A full negotiated dark-pool settlement — both parties' commitments, the `f_i` price-freshness gate, the two `open_position` CPIs into the perp engine, and the vault transfers — consumes **120,799 compute units** on a local validator, about 8.6 % of Solana's 1.4 M-CU per-transaction budget. The freshness gate itself adds only a market-PDA derivation, an 8-byte account read, and two integer comparisons; the total is dominated by the position-opening CPIs, not by the gate. Freshness gating is therefore affordable on the settlement hot path.
- *Test coverage.* The off-chain crate passes 84 tests across the default and all-oracle feature sets — exercising the per-type rejection paths, the oracle bindings (root mismatch, manifest mismatch, sub-quorum), the multi-party attribution, and the cross-language vectors. The on-chain dark-pool program passes a 96-test Anchor suite end-to-end on a local validator, including the `f_i`-gate stale-price rejection and the fresh-price clear.
- *Cross-implementation convergence.* The canonical-JSON SHA-256 construction produces byte-identical digests across the Rust, JavaScript, and TypeScript implementations on the shared test vectors (`test-vectors/v0.1.json`), now including the `f_i` input-manifest and `f_m` model-lineage roots — the empirical basis for the interoperability the wire format assumes.

These are engineering measurements on a single machine and a local validator, not a comparative benchmark; they establish feasibility and cost order-of-magnitude, which is what the economic argument of §7 requires.
...
We publish this paper to (a) establish the term *proof-of-context*, the attestation-as-settlement distinguishing axis, the four-freshness-type decomposition, the execution-context-root scope, and the cross-domain DeFi analogy on the academic record (priority date 22 April 2026, the original position-paper publication; this is revision v0.9.1 of 5 July 2026); (b) situate the contribution honestly against the substantial existing work in staleness-aware federated learning [5–9], proof-of-learning robustness [1–3, 10], centralized-provider property attestation [17], inference-activation LSH [16], and cryptoeconomic-offensive staleness [19]; and (c) invite protocol teams, independent researchers, and cryptographic-primitive designers to begin treating proof-of-context as a first-class composable settlement primitive rather than a collection of phase-specific or differently-motivated concerns.
...
**v0.9.1 (5 July 2026):** Patch revision, no new claims. (i) Corrects stale in-text version references: the §13 publication line and the License-and-Citation block still cited v0.7 / 11 June 2026 after two revisions; both now carry the current version. (ii) Records the resolution of the §10.2 non-injectivity: crate v0.3.1 encodes `AttentionImpl::Other(tag)` as the two-byte sequence `0x80 ‖ tag` (previously a seven-bit fold that collided tags differing only in the high bit); enumerated-variant roots are unchanged, no published vector had used `Other`, and regression tests pin the previously-colliding pair. §9.1's preimage-size statement gains the corresponding one-byte note. (iii) The crate additionally attributes multi-party integrity failures to the failing party (`PartyIntegrityError` carrying the party role), aligning the implementation with the per-party attribution §9.6 describes; the header version line is condensed, with history deferred to this note.

**v0.9 (21 June 2026):** Adds an empirical evaluation and a multi-party soundness result. New **§9.6** defines the multi-party gate `G^N` (a negotiated trade clears iff every party's commitment clears) and **Corollary 1** proves it FGSS-sound by a union bound over the single-party guarantee of Theorem 1 — the formal backing for the dark-pool deployment of §10.5. New **§10.6 Evaluation** reports measured numbers: a full multi-party dark-pool settlement consumes 120,799 compute units (≈8.6% of Solana's 1.4 M-CU per-transaction budget; the gate adds only a PDA derivation, an 8-byte read, and two comparisons, with the CPIs dominating); the off-chain oracle paths run in 1.1 µs (canonical hash) to 53 µs (2-of-3 quorum); the suites pass 84 crate tests and a 96-test on-chain Anchor suite; and the canonical-JSON `f_i`/`f_m` roots converge byte-identically across Rust/JS/TS. §10.4 is extended with the oracle-path costs. No claim is retracted.

**v0.8 (20 June 2026):** Brings §10 *Reference Implementation* up to the v0.3 crate and corrects two model↔implementation discrepancies. The reference gate now enforces `consistent + fresh_m + fresh_i + fresh_s` — integrity plus three of the four freshness types — where `fresh_m`/`fresh_i` are decided against the canonical-state oracle of Proposition 2 (H4), realized in two real, feature-gated forms: a witness-presented BaseOracle attestation verifier for `fresh_i`, and an M-of-N committee/quorum model-lineage registry for `fresh_m` (the trust model §7 constraint 8 leans toward). Two corrections: (i) `consistent` is refined throughout §9 — §9.2 definition, the §9.3 FGSS win condition, and Theorem 1 with its proof — to **internal triple-anchor agreement** (a property of the commit anchor alone), since the `A`-vs-`now` form conflated the integrity check with the settlement horizon and at a tight skew makes `fresh_s` unreachable; the staleness guarantee now rests on `fresh_s`, with `consistent` foreclosing a fabricated-fresh anchor (the proved soundness of the `{consistent, fresh_s}` subset is preserved in substance, not retracted). (ii) the v0.7 claim that `fresh_c` "is a block-height delta like `fresh_s`, simply not yet wired" is **withdrawn as an overclaim** — `now.h − A.h` is the commit→settle gap (`fresh_s`), not the compute→commit gap `fresh_c` names; `fresh_c` is handled structurally via commit-at-completion and omitted from the gate, and an out-of-enclave measurement would need a TEE-attested completion timestamp. New §10.5 reports a multi-party on-chain instantiation: the gate composed into an agent-to-agent perp dark pool's settlement path as a trustless `f_i` price-freshness gate plus a per-quote context commitment, generalizing the single-commitment gate to N parties. Cross-language test vectors for the `f_i` input-manifest root and the `f_m` model-lineage root were added to `test-vectors/v0.1.json`, including the array pre-sort contract.
