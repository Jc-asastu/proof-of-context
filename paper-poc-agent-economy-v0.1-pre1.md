...
**F1. Field set identity.** Every implementation produces an `_poc` block with the same field names, the same field types, and the same nesting structure. No implementation introduced a field absent from the others. No implementation omitted a required field defined by the others. Verified by direct comparison of implementation source code.

**F2. Byte-identical canonical hashes.** A single test payload, run through the canonical-hashing function in each implementation, produces the same SHA-256 hex digest. The cross-language hash equality test is currently implemented as paired tests within each implementation (each compares against an expected hex literal), not as a single multi-language test. Promotion to a single multi-language test is identified as future work in §9.
